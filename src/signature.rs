...
impl FromRequest<AppState> for VerifiedRequest {
    ...
    async fn from_request(req: Request, state: &AppState) -> Result<Self, Self::Rejection> {
        ...
        let params = match parse_from_headers(&header_map) {
            Some(params) => params,
            None => {
        ...
        let secret_key = String::from("test1234");

        // let external_host = &config.external_server_host;
        let external_host = "http://example.com";
...
pub fn parse_from_headers<'a>(header_map: &'a HeaderMap) -> Option<S3V4Params<'a>> {
...
pub fn parse_authorization_header<'a>(
    authorization: &'a str,
...
fn verify_headers_correct_secret_test() {
    ...
    assert!(verify_headers(
        &header_map,
        &parse_from_headers(&header_map).unwrap(),
        &Method::GET,
...
#[test]
fn parse_from_headers_valid_test() {
    let mut header_map = HeaderMap::new();
    header_map.insert(
        "user-agent",
        HeaderValue::from_static("aws-sdk-rust/1.1.4 os/windows lang/rust/1.71.1"),
    ...
    let out = parse_from_headers(&header_map).unwrap();
...
#[test]
fn parse_from_headers_invalid_access_key_test() {
    let mut header_map = HeaderMap::new();
    header_map.insert(
        "user-agent",
        HeaderValue::from_static("aws-sdk-rust/1.1.4 os/windows lang/rust/1.71.1"),
    ...
    assert!(parse_from_headers(&header_map).is_none());
}

#[test]
fn parse_from_headers_missing_signed_headers_test() {
    let mut header_map = HeaderMap::new();
    header_map.insert(
        "user-agent",
        HeaderValue::from_static("aws-sdk-rust/1.1.4 os/windows lang/rust/1.71.1"),
